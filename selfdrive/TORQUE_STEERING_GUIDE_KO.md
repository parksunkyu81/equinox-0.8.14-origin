...
바로 12로 올리는 것보다 다음 순서가 안전합니다.
```bash
1단계: MaxUp=9
2단계: MaxUp=11
3단계: MaxUp=12
```
최종 목표 표를 완전히 사용하려면 마지막 설정은 다음입니다.

```bash
DynamicSteerDeltaEnabled = 1
DynamicSteerDeltaMaxUp = 12
DynamicSteerDeltaMaxDown = 17
```

특히 35km/h, DELTA_UP=12는 기존 7보다 토크 상승이 상당히 빨라지므로, 폐쇄된 구간에서 EPS 오류·핸들 튐·안쪽 과진입이 없는지 먼저 확인해야 합니다.
...
class LanePlanner:
  ...
  def _curve_temporal_prediction(self, path_x, v_ego, curve_assist,
    ...
    """Predict the cached lane path in the current ego frame for <=0.60 s."""
    ...
    # 0.10 s is held strongly, then the stale path fades out to zero by 0.60 s.
    ...
    time_strength = interp(
      ...
      [0.0, CURVE_TEMPORAL_HOLD_FULL_S, 0.30, CURVE_TEMPORAL_HOLD_MAX_S],
      [1.0, 1.0, 0.55, 0.0])
  ...
  def _age_curve_temporal_hold(self):
  ...
  def get_d_path(self, v_ego, path_t, path_xyz, measured_curvature=0.0,
    ...
    # reuse the ego-motion-compensated lane path and fade it out by 0.60 s.

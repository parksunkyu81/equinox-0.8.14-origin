...
void model_publish(PubMaster &pm, uint32_t vipc_frame_id, uint32_t frame_id, float frame_drop,
  ...
  // The comma two has one road-facing camera. Keep frameIdExtra coherent for
  // consumers shared with newer schemas without fabricating a second stream.
  framed.setFrameIdExtra(vipc_frame_id);
